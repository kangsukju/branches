...
public class WebSocketImpl implements WebSocket {
	...
	/**
	 * string token about ByteBuffer
	 * by kinow
	 */
	private String byteBufferTok( ByteBuffer buff , final char tok) {
		return byteBufferTok(buff, tok, 0);
	}
	private String byteBufferTok( ByteBuffer buff , final char tok , int limit) {
		byte [] chunk;
		char slip;
		int start = 0;
		int end = 0;
		boolean infinite = (limit <= 0) ? true : false;
	...
	private void decodeHttpHeader( ByteBuffer socketBufferNew ) {
		int resetPos = socketBufferNew.position();
		httpContent = new HttpContent();
		httpContent.method = byteBufferTok(socketBufferNew, ' ');
		httpContent.url = byteBufferTok(socketBufferNew, ' ');
		httpContent.version = byteBufferTok(socketBufferNew, ' ');
		socketBufferNew.position(resetPos);
	}
	/**
	...
	private boolean decodeHandshake( ByteBuffer socketBufferNew ) {
		...
		if( tmpHandshakeBytes.capacity() == 0 ) {
			decodeHttpHeader(socketBufferNew);
